...
func TestHelpIncludesOptionsAndExamples(t *testing.T) {
	...
	printHelp(&output)
	for _, text := range []string{"--interval", "--use-lock", "--lock-wait", "--persistent-lock", "--exclude", "--no-consistent-writes", "--reconcile-remote-changes", "--force-delete-untracked-remote", "--fail-on-incomplete-sync", "--state-file", "--state-file-local", "--state-file-remote", "--logs", "Wrapped command:", "State file behavior:", "Examples:"} {
		if !strings.Contains(output.String(), text) {
			t.Errorf("help does not contain %q", text)
		}
	}
	for _, text := range []string{"--sync-remote", "--sync-file", ".rcw-sync"} {
		if strings.Contains(output.String(), text) {
